...
// Light two-column: pale rail with accent left-border, minimal color.
export default function Crisp({ cv }) {
  ...
  return (
    <div className="cv-page mx-auto flex" style={{ fontFamily: font, color: "#1a202c" }}>
      <aside className="w-[32%] p-7" style={{ background: "#f8fafc", borderLeft: `3px solid ${color}` }}>
        <div className="text-[20px] font-extrabold leading-tight">{pi.full_name || "Your Name"}</div>
        {pi.professional_title ? <p className="text-[12px] mt-1 mb-4" style={{ color }}>{pi.professional_title}</p> : <div className="mb-4" />}
        <ContactLine pi={pi} className="text-[10.5px] text-gray-600 leading-relaxed mb-5" />
        {order.filter((k) => sideKeys.includes(k)).map(side)}
      </aside>
      <main className="flex-1 p-7">{order.filter((k) => !sideKeys.includes(k)).map(main)}</main>
    </div>
  );
}